...
export default function Home() {
  ...
  // ====== UNIFIED STORY STATE & TYPING (replaces 4 booleans + 4 typed buffers) ======
  /** @typedef {'disco'|'shocked'|'pepeShh'|'party'|null} StoryKey */
  const [activeStory, setActiveStory] = useState(/** @type {StoryKey} */ (null));
  ...
  const STORY_TEXT = {
    disco: fullStory,
    shocked: shockedStory,
    pepeShh: pepeShhStory,
    party: partyStory,
  };

  function scheduleStory(next, delay = 800) {
  ...
  function clearStory(key) {
    if (revealTimerRef.current) {
      clearTimeout(revealTimerRef.current);
      revealTimerRef.current = null;
    }
    if (key && activeStory === key) {
      setActiveStory(null);
    }
  }

  // Single typing loop that restarts on activeStory change
  useEffect(() => {
    ...
    const text = STORY_TEXT[activeStory];
    setTyped("");
    let i = 0;
  ...
  }, [activeStory]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    const partyHeroEl =
      partyHeroRef.current || document.getElementById("party-hero");
    if (partyHeroEl) {
    ...
    return () => {
      ...
      // NEW: kill any pending timers
      if (revealTimerRef.current) clearTimeout(revealTimerRef.current);
      if (typeTimerRef.current) clearInterval(typeTimerRef.current);
    };
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  // Compute the exact midpoint between "PEPE'S" and "PARTY"
  useEffect(() => {
  ...
  return (
    <div
      className={`relative bg-black overflow-x-hidden ${
        isBW ? "overflow-hidden grayscale" : "overflow-y-auto"
      }`}
    >
      ...
      >
        ...
        >
          <div className="bg-black/40 backdrop-blur-xl backdrop-saturate-150 text-white px-10 py-3 rounded-full shadow-[0_0_25px_#a855f7] flex gap-10 items-center justify-center">
            ...
            >
              <span className="text-xl font-semibold group-hover:text-purple-400 transition-all duration-200">
                Links
              </span>
            </button>
              ...
              <div className="absolute bottom-4 left-0 w-full px-4 flex items-center justify-center gap-2 z-30">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-purple-200 text-center font-bold leading-tight">
                    DISCO IS PART OF THE HEDZ
                    <br />
                    COLLECTION BY MATT FURIE.
                  </span>
                </div>
                <img
                  src="/verified.gif"
          ...
          <section className="relative h-screen w-full overflow-hidden">
            ...
            <div id="second-section-content" className="w-full h-full relative z-10">
              ...
              <img
                ...
                style={{ transform: "scaleY(-1) translateX(80px)" }}
              />
        ...
        {!isBW && (
          <section ref={shockedRef} className="relative h-screen w-full bg-black overflow-hidden">
            <motion.div className="absolute top-0 left-0 w-full h-full" style={{ scale }}>
        ...
        {!isBW && (
          <section ref={pepeShhRef} className="relative h-screen w-full bg-black overflow-hidden">
            <motion.div className="absolute top-0 left-0 w-full h-full" style={{ scale: pepeShhScale }}>
              {/* Moving clouds background */}
              <div
          ...
          >
            ...
            <div className="relative h-screen flex items-center justify-center w-full">
              ...
              >
                ...
                {/* Word under */}
                <span ref={wordRightRef} className="text-purple-400 whitespace-nowrap inline-block relative z-[120]">
                  PARTY
                </span>
              </h1>
            </div>
            ...
            <div id="party-hero" ref={partyHeadingRef} className="relative h-screen w-full overflow-hidden isolate">
              ...
              {/* 🖼️ Party hero art */}
              <img src="/discopartybrett.png" alt="Party Hero" className="absolute inset-0 w-full h-full object-cover z-10" />
        ...
        {/* ✅ STORYLINE POPUP (desktop: centered at bottom) — unified */}
        <motion.div
          initial={{ y: 100, opacity: 0 }}
          animate={activeStory ? { y: 0, opacity: 1 } : { y: 100, opacity: 0 }}
        ...
        >
          {activeStory && (
            <div className="pointer-events-auto bg-black/80 backdrop-blur-xl border border-purple-500/40 shadow-[0_0_25px_#a855f7] rounded-xl px-6 py-4 flex items-center gap-4 w-auto max-w-3xl mx-auto">
              {activeStory === "disco" && (
                <>
                  <div className="flex-shrink-0 text-purple-400 text-lg font-extrabold tracking-wide">DISCO?</div>
                  <div className="w-px h-10 bg-purple-500/40" />
                  <div className="flex-1 text-left text-sm font-medium text-purple-200">
                    <span>{typed}</span>
                    <span className="text-purple-500/40">{STORY_TEXT.disco.slice(typed.length)}</span>
                  </div>
                </>
              )}
              ...
              {activeStory === "shocked" && (
                <div className="flex-1 text-left text-sm font-medium text-purple-200">
                  <span>{typed}</span>
                  <span className="text-purple-500/40">{STORY_TEXT.shocked.slice(typed.length)}</span>
                </div>
              )}

              {activeStory === "pepeShh" && (
                <>
                  <div className="flex-shrink-0 text-green-400 text-lg font-extrabold tracking-wide">PEPE</div>
                  <div className="w-px h-10 bg-green-500/40" />
                  <div className="flex-1 text-left text-sm font-medium text-green-200">
                    <span>{typed}</span>
                    <span className="text-green-500/40">{STORY_TEXT.pepeShh.slice(typed.length)}</span>
                  </div>
                </>
              )}
              ...
              {activeStory === "party" && (
                <>
                  ...
                  <div className="flex-1 text-left text-sm font-medium text-green-200">
                    <span>{typed}</span>
                    <span className="text-green-500/40">{STORY_TEXT.party.slice(typed.length)}</span>
                  </div>
                </>
              )}
            </div>
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <a href="https://x.com/discohedzeth" target="_blank" rel="noreferrer" className="group">
                      <div className="w-32 h-32 rounded-2xl bg-black flex items-center justify-center 
                              transition-transform duration-300 group-hover:scale-110 shadow-[0_0_25px_#a855f7]">
                        <img src="/icons/x.svg" alt="X" className="w-16 h-16 transition-transform duration-300 group-hover:scale-125" />
                      </div>
                    </a>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
                      ...
                      <div className="rounded-3xl bg-black/40 backdrop-blur-xl border border-purple-500/30 shadow-[0_0_25px_#a855f7] p-6 row-span-2 flex items-center justify-center relative overflow-hidden">
                        <img src="/disco-body.png" alt="Disco Body" className="w-full h-full object-contain scale-[1.5] origin-center" />
                      </div>
                      ...
                      <div className="relative rounded-3xl bg-black/40 backdrop-blur-xl border border-purple-500/30 shadow-[0_0_25px_#a855f7] p-6 lg:col-span-2 h-full overflow-hidden">
                        <img src="/discopartybrett.png" alt="Matt Furie Art" className="absolute inset-0 w-full h-full object-cover opacity-70" />
                        <div className="rounded-3xl bg-black/40 backdrop-blur-xl border border-purple-500/30 shadow-[0_0_25px_#a855f7] p-6 h-[300px] relative overflow-hidden">
                          ...
                          <div className="relative z-10 flex flex-col text-center h-full">
                            ...
                            <div className="flex flex-1 items-center justify-center">
                              <div className="text-3xl md:text-4xl font-extrabold text-white">The Last Dance</div>
                            </div>
                          </div>
                        </div>
                      </div>
